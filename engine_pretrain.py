...
def train_one_epoch(model: torch.nn.Module,
    ...
    for data_iter_step, (samples, _) in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        # Autocast only if device is CUDA
        autocast_ctx = torch.amp.autocast(device_type='cuda', enabled=(device.type=='cuda'))
        with autocast_ctx:
            loss, loss_contrastive, loss_noise, _, _ = model(samples, mask_ratio=args.mask_ratio)

        # Handle loss_noise if not used
        if isinstance(loss_noise, int):
        ...
        # Reduce across processes if using distributed (CPU safe)
